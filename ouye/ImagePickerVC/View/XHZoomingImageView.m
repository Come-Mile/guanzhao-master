...
- (void)setImage:(id )image {
    ...
    if ([image isKindOfClass:[UIImage class]]) {
        ...
        [self setUpIMagforProgressWithImageStr:image];
    }
    
}

- (void)setUpIMagforProgressWithImageStr:(NSString *)imageStr
{
    __block UIActivityIndicatorView *activityIndicator;
    __block XHZoomingImageView *xhView = self;
    // 这里是在给imageView附上图片的.
    [xhView.imageView sd_setImageWithURL:[NSURL URLWithString:imageStr] placeholderImage:[UIImage imageNamed:@"zhan"] options:SDWebImageProgressiveDownload progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        if (!activityIndicator)
        {
            [xhView.imageView addSubview:activityIndicator = [UIActivityIndicatorView.alloc initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray]];
            activityIndicator.center = xhView.imageView.center;
