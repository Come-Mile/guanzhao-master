...
+ (id<TYAppendTextStorageProtocol>)parseTextStorageFromDictinary:(NSDictionary *)dic
{
    ...
    if (fontSize > 0) {
        ...
        textStorage.font = [UIFont systemFontOfSize:16.0f];
    }
...
//    imageStorage.imageName = dic[@"name"];
    imageStorage.imageURL =[NSURL URLWithString:dic[@"name"]];
    imageStorage.placeholdImageName = @"logo";
    //[dic[@"width"] doubleValue]
    imageStorage.size = CGSizeMake(ScreenW, [dic[@"height"] doubleValue]);
