...
+ (void)showAdAtPath2:(NSString *)path onView:(UIView *)container timeInterval:(NSTimeInterval)interval detailParameters:(NSDictionary *)param placeHoderImage:(NSString *)place
{
    if (![self validatePath:path]) {
        return ;
    }
    ...
    [self showImageOnView2:container forTime:interval path:path placeImageName:place];
}
...
+ (void)showImageOnView2:(UIView *)container forTime:(NSTimeInterval)time path:(NSString *)path placeImageName:(NSString *)placeHolderImgName
{
    CGRect f = [UIScreen mainScreen].bounds;
    NSLog(@"screen size:%@", NSStringFromCGRect(f));
    UIView *v = [[UIView alloc] initWithFrame:f];
    ...
    if (!(path.length > 0)) {
        ...
        iv.image = [UIImage imageNamed:placeHolderImgName];
    ...
    if (isImgAlreadyExists) {
        ...
        UIImage *image = [manager.imageCache imageFromDiskCacheForKey:path];
        iv.image = image;
        
    }else{
        [iv sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:placeHolderImgName]];
        MWLog(@"下载loading：%@",iv.image);
    }
...
//        return;
      ...
      [self.imageV sd_setImageWithURL:url placeholderImage:[UIImage imageNamed:placeHolderImgName]];
